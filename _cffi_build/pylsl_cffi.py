...
def get_lsl_libpath():
    ...
    libname = 'lsl' + (str(bitness) if bitness == 64 else "")
    ...
    libpath = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'pylsl', 'lib', libprefix+libname+libsuffix))
...
def get_header_path():
    ...
    return os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'pylsl', 'include'))
...
def get_cleaned_header():
    ...
    # Cut the #defines and #ifdefs
    header_dat = re.sub(r"(?m)[\t\f\v ]*\#.*\n?", r"", header_dat)
    header_dat = re.sub(r'(?m)^extern \"C\" {.*\n?', r"", header_dat)
    header_dat = re.sub(r'(?m)^} \/\*.*\n?', r"", header_dat)
